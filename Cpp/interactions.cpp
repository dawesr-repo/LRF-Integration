      ...
      for (int order = 1; order <=15; ++order){
          if ( m_fit_[order-1] > 0){
                multipole_sph =  multipole_sph + (C3 * C1 * pow(C2,order)) *
                                 MultipoleOrder(order,t_tensors) / pow(r,order);
...
const double PotentialEnergySurface::InductionInteraction(const double &r,const vector <double> &t_tensors){
  ...
  for (int order = 1; order <=15; ++order){
    if ( i_fit_[order-1] > 0){
      ...
      induction_sph = induction_sph + InductionOrder(order,r,t_tensors,1); // induction of B over A
...
const double PotentialEnergySurface :: InductionComponent( const int &i,
  ...
  const auto pol_arr = index == 1? b_pol_[lmin-1][lmax-1]: a_pol_[lmin-1][lmax-1];
            ...
            if ( abs(qbj) > EPS ){
               for (int k1 = 1; k1 <=nl1;++k1){
                   for (int k2 = 1;k2 <= nl2;++k2){
                     const int cpn = l1 > l2 ? (k2-1) * (2*l1+1) + k1: (k1-1) * (2*l2+1) + k2;
...
const double PotentialEnergySurface::DispersionOrder(const double &r,
                                                     const vector <double> &t_tensors,
                                                     const int &order){
...
const double PotentialEnergySurface::DispersionComponent(const int &l1,const int &l2,
      ...
      for (int  lj = 0; lj<=2*l2;++lj){
          for (int  ti = 0; ti<=2*t1; ++ti){
              for (int tj = 0; tj<=2*t2 ; ++tj){
                 ...
                 const int cpn = (l1>l2 && t1>t2) ? lj * (2*l1+1) * (2*t2+1) * (2*t1+1) + li * (2*t2+1) * (2*t1+1) +  tj * (2*t1+1) + ti+1 :
                                 (l1>l2 && t1<=t2 ) ? lj * (2*l1+1) * (2*t1+1) * (2*t2+1) + li * (2*t1+1) * (2*t2+1) +  ti * (2*t2+1) + tj+1 :
                                 (l1<=l2 && t1>t2 ) ? li * (2*l2+1) * (2*t2+1) * (2*t1+1) + lj * (2*t2+1) * (2*t1+1) +  tj * (2*t1+1) + ti+1 :
                                  li * (2*l2+1) * (2*t1+1) * (2*t2+1) + lj * (2*t1+1) * (2*t2+1) +  ti * (2*t2+1)+ tj +1;
                ...
                if ( abs(disp_coeff) > EPS ) {
                  ...
                  res = res + disp_coeff * t_tensors.at(t_cpn_1) *
                                           t_tensors.at(t_cpn_2);
